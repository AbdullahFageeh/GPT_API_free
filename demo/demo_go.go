...
func gpt35APIStream(ctx context.Context, client *openai.Client, messages []openai.ChatCompletionMessage) error {
	...
	// Optimization: Use os.Stdout.WriteString to avoid fmt.Print's reflection and interface boxing overhead.
	// Localize os.Stdout for even faster access in the hot loop.
	stdout := os.Stdout
	for {
