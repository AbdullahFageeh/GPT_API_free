...
func gpt35APIStream(ctx context.Context, client *openai.Client, messages []openai.ChatCompletionMessage) error {
    ...
    // Localize os.Stdout to a local variable to avoid repeating package-level lookups
    stdout := os.Stdout
    ...
    for {
        ...
        if len(response.Choices) > 0 {
            // Using stdout.WriteString avoids reflection and interface{} boxing overhead on every token iteration
