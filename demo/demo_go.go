...
func gpt35APIStream(ctx context.Context, client *openai.Client, messages []openai.ChatCompletionMessage) error {
	...
	for {
		...
		if len(response.Choices) > 0 {
			// Using os.Stdout.WriteString for better performance in streaming loops.
			// os.Stdout.WriteString is faster than fmt.Print as it avoids reflection and internal buffering.
